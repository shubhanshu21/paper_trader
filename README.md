...
- Paper trading and live deployment run through the same in-process scheduler (`api/strategy_scheduler.py`) — no separate daemon process.
...
**Backtest.** `backtest/custom_engine.py` walks every real historical expiry cycle in MySQL's `fno_bhavcopy` table for the chosen date range (user-selectable in the UI — quick presets or an exact From/To), driving the same `RuleBasedStrategy` through `MockBroker`. Real transaction costs (`utils/costs.py`) are netted out of every cycle; cycle discovery respects the strategy's weekly/monthly expiry choice.

**Paper & live.** ONE background asyncio task for the whole API process — `api/strategy_scheduler.py`, started in `main.py`, not a separate daemon/service — ticks every `PAPER_TRADING`/`LIVE`/`PAUSED` strategy (any `strategy_type`, dispatched to the right engine — see "Which Engine Runs a Strategy" below) each pass during market hours (every 30s while the market's open, 300s while it's closed). For the leg-based engine specifically: entry is gated per **expiry cycle**, not calendar day — if a position exits early (TP/SL hit), it won't re-enter the same soon-to-expire contract the next morning, it waits until the resolved expiry actually rolls over. Paper P&L nets real transaction costs too (not just backtest) — slippage is applied on both paths via `PaperBroker`/`MockBroker`.
...
### Which Engine Runs a Strategy

Every strategy has a `strategy_type`. `strategies/custom/engine_registry.py` maps it to the right validate/describe/backtest behavior; `api/strategy_scheduler.py` maps it to the right tick function. **`CUSTOM` (and any legacy/unknown type) always falls through to the leg-based engine above** — that's deliberate: it's what lets every strategy that's just a new combination of buy/sell legs get built with **zero new backend code**, ever. A genuinely new `strategy_type` (its own schema + executor + tick function, all under `strategies/custom/` and `api/`) only gets created when a strategy's *execution model* — not just its legs — doesn't fit the leg-based engine's "resolve legs once, enter once, hold, exit once" shape. Six such cases exist today:

| `strategy_type` | Why it couldn't just be a leg-based template |
|---|---|
| `SUPERTREND_INTRADAY` | Enters/exits several INDEPENDENT times within a single day, off live 5-min candles — the leg-based engine has no concept of multiple entries per expiry cycle. |
| `WEEKEND_GAP_COMBO` | ONE combined position spans TWO DIFFERENT symbols (NIFTY + SENSEX) with a single shared target/stop — the leg-based engine is single-symbol-per-strategy by design. |
| `OTM_PUT_ROLL` | REPLACES its one open leg with a new one at a different strike, several times, without ever fully exiting the expiry cycle — "roll" has no equivalent in "exit once." |
| `SMART_CONDOR` | A live cross-leg premium-RATIO trigger swaps one side's short+hedge pair for a new pair mid-cycle, capped at N adjustments — needs runtime state the static engine doesn't track. |
| `GRAVITY` | A STATEFUL daily-close signal (price must breach a level, THEN a later candle must close back inside it) with strike selection from a live-tracked extreme, not a fixed offset. |
| `HAI_WEEKLY_1_3_2_CALL_RATIO`, `NO_DECAY_WEEKLY_RATIO_SPREAD`, `WEEKLY_HEDGED_PUT_RATIO`, `OVERHEDGED_MONTHLY_IRON_FLY`, `MONTHLY_BATMAN_1_3_2` | **Not** new engines — these ARE `strategy_type: CUSTOM` templates (see below), included here only to be clear they're leg combinations, not exceptions to the rule above. |

**Rule of thumb for extending this app**: if a new strategy idea is "different strikes/lots/entry-exit-thresholds on the same buy/sell-legs-once shape," it needs a template (below), not code. If it needs the engine itself to track state across ticks, replace a leg without exiting, or span multiple symbols as one position — it needs a new engine, following the pattern of the six above.

### Available Strategy Templates

`GET /api/custom-strategies/templates/strategy-types` (`api/routes_custom_strategies.py::get_strategy_types()`) is what the builder's "Start from a template" step actually reads — every template below is a REAL, ready-to-use `legs` array (validated by `rule_schema.validate_rules()` in CI, see `tests/test_strategy_templates.py`), not a display-only mockup. Picking one pre-fills the builder; every field — strikes, lots, entry/exit — stays editable afterward, template or not.

| Template | What it is | When to use it | Why |
|---|---|---|---|
| **`STRADDLE`** | BUY ATM CE + BUY ATM PE | You expect a BIG move but don't know which direction (pre-earnings, pre-budget, pre-RBI-policy). | Simplest pure-volatility bet — unlimited profit either direction, loses only if the underlying stays pinned near the strike through expiry. Max loss is capped at the combined premium paid. |
| **`STRANGLE`** | BUY 5% OTM CE + BUY 5% OTM PE | Same big-move thesis as a Straddle, but you want to pay less premium upfront and can accept needing a bigger move to profit. | Cheaper entry than a Straddle (both legs start further from spot), at the cost of a wider "dead zone" where both legs expire worthless. |
| **`IRON_CONDOR`** | SELL 5% OTM PE + BUY 10% OTM PE + SELL 5% OTM CE + BUY 10% OTM CE | You expect the underlying to stay RANGE-BOUND through expiry and want defined, capped risk (not naked selling). | Classic income strategy: collects net credit from the two short legs, the two long legs cap the maximum loss on either side — you know your worst case before you enter. |
| **`BUTTERFLY`** | BUY ATM CE + SELL 2x (5% OTM CE) + BUY 10% OTM CE | You expect the underlying to PIN near a specific strike by expiry (low realized volatility). | Very low cost (the short legs' premium subsidizes the long ones), very low defined risk — but also low max profit, and needs the underlying to land close to the middle strike, not just "somewhere in range." |
| **`OVERHEDGED_MONTHLY_IRON_FLY`** | 1-lot ATM straddle (long) + 2-lot strangle offset by half the straddle's live premium (short) + 2-lot deep-OTM ₹40-70 insurance each side | Monthly, low-touch income — you want an Iron Fly's income profile but with EXTRA tail-risk insurance beyond the usual single hedge. | The "over-hedge" (2 lots of cheap deep-OTM protection per side, not just 1) is what makes this different from a textbook Iron Fly — caps a gap-risk/black-swan move harder, at a small extra cost. Exits on a flat ₹4,500 profit or the position's own computed breakeven, whichever comes first — not a fixed %. |
| **`MONTHLY_BATMAN_1_3_2`** | 1:3:2 call+put ratio spreads (both sides), sized to the "Batman logo" payoff shape | Monthly, non-directional income, comfortable actively reviewing the net credit and T+0 curve before EACH cycle (not fully hands-off). | The 1:3:2 ratio (buy 1, sell 3, buy 2 as a further hedge) gives a wide, symmetric profit plateau — but needs manual per-cycle checks (net credit under ~6% of capital; balance the T+0 curve) the engine itself doesn't automate, unlike the fully hands-off templates. |
| **`NO_DECAY_WEEKLY_RATIO_SPREAD`** | SELL 1-lot ATM straddle + BUY 2-lot OTM CE + BUY 2-lot OTM PE (~200-300pts) | Weekly, want SOME downside-of-flat-market protection from theta decay while still profiting hard on a sharp break either direction. | The double-quantity long wings mean a big move lets the longs "outrun" the short — net long gamma overall, unlike a plain short straddle. Real caveat: there's still a loss zone between the ATM short and the OTM longs on a MODERATE (not flat, not sharp) move — inherent to the shape, which is why a capital-%-based stop-loss is on by default. |
| **`WEEKLY_HEDGED_PUT_RATIO`** | BUY 1-lot near-OTM PE + SELL 2-lot further-OTM PE + BUY 1-lot deep-OTM PE | Weekly (or monthly), want a high-probability-of-profit, fully-hedged put-selling income structure with low margin usage vs. naked put-selling. | Every leg is hedged (no naked short), so margin stays well below a plain cash-secured put seller's — and the payoff PEAKS if the underlying drifts down toward the sold strike, which also cushions a late gamma spike a naked seller would get hurt by. |
| **`HAI_WEEKLY_1_3_2_CALL_RATIO`** | BUY 1-lot 200pt-OTM CE + SELL 3-lot 400pt-OTM CE + BUY 2-lot 600pt-OTM CE, entered Monday for next week's expiry, exits Friday | Weekly, all-CALL (so a downside gap/crash literally cannot hurt you beyond a small net credit/debit — every leg is a call), zero mid-week adjustments wanted. | The 1(bought)+2(bought)=3(sold) balance caps the UPSIDE risk too (unlike a naked call ratio spread) — this is a strictly rule-based, no-adjustment weekly income strategy, entered/exited on a fixed calendar schedule (`entry.weekday`/`exit.exit_weekday` — see `rule_schema.py`) rather than a floating one. |
| **`CUSTOM`** | A single starter leg | None of the above match your idea, or you're building something genuinely new from scratch. | Every field is yours to set — any combination of up to 8 legs, any strike-selection mode, any entry/exit rule. This is the same generic shape every template above is built from; nothing a template can do is off-limits here. |

### The Five Non-Leg-Based Engines

These `strategy_type`s are created directly (not from a builder template — their rules shape isn't a `legs` array at all), each because its execution model is genuinely different — see the table above for the one-line "why." What each is actually FOR:

- **`SUPERTREND_INTRADAY`** (`strategies/custom/intraday_schema.py`) — a pure intraday signal strategy: sells the ATM PUT when price closes above both Supertrend and pivot R1 (bullish), or the ATM CALL on the bearish mirror, off 5-minute candles, exiting same-day on a Supertrend flip or a hard time cutoff. **Use when** you want a mechanical, indicator-driven day-trading strategy with no overnight exposure at all — this is the only engine that can take several independent trades in one day.
- **`WEEKEND_GAP_COMBO`** (`strategies/custom/combo_schema.py`) — enters a NIFTY ratio spread + an opposite-direction SENSEX ratio spread together every Friday afternoon, targeting the historical post-weekend gap tendency, exits the following week with one shared target/stop across both legs' combined P&L. **Use when** you have a directional view for Monday's opening gap and want to express it across two correlated indices at once, hedged against each other by construction.
- **`OTM_PUT_ROLL`** (`strategies/custom/otm_put_roll_schema.py`) — sells a far-OTM, far-month NIFTY put only after a genuine pullback off a recent high, then mechanically ROLLS the strike down (closes, re-sells lower) every time spot closes below it, up to a safety cap. **Use when** you want a slow, trend-following short-put income strategy that actively defends itself as the market falls, instead of sitting still until a hard stop.
- **`SMART_CONDOR`** (`strategies/custom/smart_condor_schema.py`) — an Iron Condor whose short strikes are set from the LIVE ATM straddle premium (not a fixed points offset) each Monday, and which RE-CENTERS itself (closes+re-sells the cheaper side to match the threatened side's premium) up to twice per week before forcing a full exit on a third breach. **Use when** you want a weekly Iron Condor that actively defends itself against one-sided moves instead of just eating the loss or exiting outright.
- **`GRAVITY`** (`strategies/custom/gravity_schema.py`) — trades FAKEOUTS: waits for the underlying to breach its own monthly Camarilla R3/S3 pivot, then confirms with a later daily candle closing back inside it, and sells an OTM credit spread near that fakeout's own extreme. **Use when** you're trading a single, liquid, low-circuit-risk stock (built around Reliance) and want to fade failed breakouts/breakdowns rather than trade the breakout itself, with a defined, hedged 2-leg spread instead of a naked option.
...
- **Single database for the whole bot.** `positions.db` and a separate, PaperBroker-only `paper_trades.db` (leg-level, consumed only by the now-deleted `run_paper_tracker.py`) are unified into one `positions` table — the AuditTrail (`logs/audit_trail.log`) already independently records every order attempt, so the second SQLite log was a pure duplicate. (Originally one SQLite file, `data/runtime/trading.db`; since moved to MySQL — see `DatabaseConfig` in `config.py` — for query speed at scale. The app is MySQL-only end to end now, including the test suite — see tests/conftest.py.)
...
- **No automated backup for the MySQL `fno_bhavcopy` table** (~27GB of data) — rebuildable from source (Kaggle archive + NSE's public bhavcopy + live downloads) but that takes real time.
