...
Usage:
    python3 -m cli.run_daemon                            # everything from .env — this is the normal cron invocation
    python3 -m cli.run_daemon --strategies ten_percent_otm_strangle
...
timing, nothing about paper/live:
    @reboot cd /path/to/automate && nohup .venv/bin/python -m cli.run_daemon >> logs/daemon.log 2>&1 &
...
from cli.run_position_monitor import monitor_once
from cli.run_strategy import run_entries
...
from config import STRATEGY_CONFIGS, LogConfig, RunConfig, UpstoxConfig
from strategies.registry import STRATEGIES
from utils.logger import setup_logger
from utils.notify import notify
from utils.position_tracker import get_open_positions
from utils.strategy_overrides import get_effective_config
from utils.telegram_alert import alert_heartbeat
...
def _ensure_fresh_upstox_token_bounded(log: logging.Logger) -> str | None:
    ...
    try:
        ...
        notify(
            ...
            f"cycle. If this keeps happening, check manually with `python3 -m auth.upstox_auto_login`.",
        )
        return None
    finally:
...
def main() -> int:
    ...
    # having run `python3 -m auth.upstox_auth` first. Bounded so a
