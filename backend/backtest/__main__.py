...
name, type (index/stock), date range. That's it.
    ...
    python3 -m backtest --symbol NIFTY --type index --from 2015-01-01 --to 2020-12-31
...
# backend/backtest/__main__.py -> up 2 levels to backend/ (where logs/,
# cache/, data/, scripts/ actually live) — subprocess calls below need
# this as their cwd. No sys.path hack needed: `backend` is installed
...
def main():
    ...
    if from_d >= today - timedelta(days=LIVE_DATA_MAX_DAYS):
        ...
        if rc != 0:
            ...
            return rc
        cmd = [sys.executable, "-m", "backtest.engine", "--symbol", symbol, "--strategy", args.strategy, "--num-lots", str(args.num_lots)]
        ...
        cmd = [
            sys.executable, "-m", "backtest.historical_engine",
