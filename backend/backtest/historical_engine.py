...
nothing to keep in sync by hand. Also run directly as a CLI:
    ...
    python3 -m backtest.historical_engine --symbol NIFTY
    python3 -m backtest.historical_engine --symbol BANKNIFTY --strike-step 100
...
def print_trade_details(results: list[dict], show_exit_reason: bool = False) -> None:
    ...
    from utils.table import render_stats_table
...
def print_summary_table(all_results: list[dict], liquid_results: list[dict], meta: dict) -> None:
    ...
    from utils.table import print_meta_line, render_stats_table
