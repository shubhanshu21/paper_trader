...
def find_monthly_expiries(
    expiries: list[str], fmt: str = "%Y-%m-%d", reference_date: date | None = None,
) -> list[str]:
    """
    Every upcoming MONTHLY expiry (see find_nearest_expiry_by_type's own
    docstring for the "last listed expiry in its calendar month" rule),
    sorted ascending — index 0 is the nearest monthly, index 1 is the
    NEXT one after that ("far month"), etc. Used by strategies that
    deliberately skip the nearest monthly for a further-out one (more
    premium cushion, slower initial theta) — find_nearest_expiry_by_type
    alone only ever gives index 0.

    Returns [] if there are no upcoming monthly expiries at all (same
    "don't guess, just report nothing available" contract as the rest of
    this module).
    """
    today = reference_date if reference_date is not None else date.today()
    parsed: list[tuple[date, str]] = []
