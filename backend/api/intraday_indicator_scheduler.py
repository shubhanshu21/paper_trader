"""
api/intraday_indicator_scheduler.py — the SIGNAL-DRIVEN engine for
Supertrend+Pivot intraday strategies (CustomStrategy rows with
strategy_type == "SUPERTREND_INTRADAY", see strategies/custom/
intraday_schema.py).

This is a SEPARATE engine from custom_strategy_scheduler.py, not an
extension of it, because this strategy shape breaks that engine's core
assumption: "enter once per expiry cycle, hold, exit once." Supertrend+
Pivot strategies can enter/exit several INDEPENDENT times within a
single trading day, with the option_type decided live by a signal
...
CustomStrategy row (any strategy_type) and dispatches SUPERTREND_INTRADAY
