...
from utils.costs import sum_breakdowns

router = APIRouter(prefix="/api/backtest", tags=["backtest"])

REPO_ROOT = Path(__file__).resolve().parent.parent  # backend/ — the flat package root, needed as subprocess cwd for `-m backtest.engine`/etc. below to resolve
...
def _run_historical(req: BacktestRequest, contract_type: str) -> dict:
    from backtest.historical_engine import HistoricalCycleEngine
    from compliance.sebi_rules import init_market_calendar
    ...
    if stats_all["cycles"] > 0:
        from utils.backtest_history import record_backtest_run
        record_backtest_run(
...
def _run_recent(req: BacktestRequest, contract_type: str) -> dict:
    ...
    cmd = [sys.executable, "-m", "backtest.engine", "--symbol", req.symbol.upper(),
