...
# ---------------------------------------------------------------------------
# Load .env from backend/ (self-contained, like frontend/'s own env files).
# backend/config/__init__.py -> up 2 levels (config -> backend).
...
def _load_access_token() -> str:
    ...
    """
    try:
        from auth.token_store import get_access_token
...
def _save_access_token(token: str) -> None:
    from auth.token_store import set_access_token
...
class PanelAuthConfig:
    ...
    def jwt_secret(cls) -> str:
        ...
        secret_file = Path(__file__).resolve().parent.parent / ".panel_jwt_secret"  # backend/.panel_jwt_secret
