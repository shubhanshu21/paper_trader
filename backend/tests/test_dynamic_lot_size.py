...
class TestStrategyRefusesToGuess:
    def test_raises_when_broker_cannot_resolve_lot_size(self):
        ...
        with (
            patch("utils.instrument_cache.InstrumentCache.resolve_equity_key", return_value="NSE_EQ|TEST"),
    ...
    def test_raises_when_broker_cannot_resolve_strike_step(self):
        ...
        with (
            patch("utils.instrument_cache.InstrumentCache.resolve_equity_key", return_value="NSE_EQ|TEST"),
            patch("broker.mock_broker.MockBroker.get_lot_size", return_value=500),
            patch("broker.mock_broker.MockBroker.get_strike_step", return_value=None),
