...
class TestMarketStatus:
    def test_returns_closed_with_a_reason_outside_market_hours(self, monkeypatch):
        ...
        monkeypatch.setattr("compliance.sebi_rules.assert_market_is_open", _raise)
    ...
    def test_returns_open_when_market_is_open(self, monkeypatch):
        monkeypatch.setattr("compliance.sebi_rules.assert_market_is_open", lambda: None)
    ...
    def test_never_raises_even_if_the_calendar_check_itself_errors(self, monkeypatch):
        ...
        monkeypatch.setattr("compliance.sebi_rules.assert_market_is_open", lambda: (_ for _ in ()).throw(RuntimeError("boom")))
        result = main_module.market_status()
        assert result["open"] is False
