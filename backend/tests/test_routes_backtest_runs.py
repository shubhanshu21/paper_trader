...
import api.routes_custom_strategies as routes
from api.routes_custom_strategies import (
    BacktestRequest,
...
from db.models import CustomBacktestRun, CustomStrategy
...
class TestRunBacktestSymbols:
    def test_merges_and_tags_cycles_chronologically(self):
        with patch("backtest.custom_engine.CustomRuleBacktestEngine", FakeEngine):
...
class TestRunBacktestSync:
    def test_completes_and_populates_result(self, db, strategy, session_local_patch):
        ...
        with patch("backtest.custom_engine.CustomRuleBacktestEngine", FakeEngine), \
             patch("backtest.custom_engine.compute_nifty_benchmark_return", return_value=6.0):
    ...
    def test_progress_updates_during_run(self, db, strategy, session_local_patch):
        ...
        with patch("backtest.custom_engine.CustomRuleBacktestEngine", FakeEngine), \
             patch("backtest.custom_engine.compute_nifty_benchmark_return", return_value=None):
            _run_backtest_sync(run.id)
