...
# the `db.engine` package ATTRIBUTE with the Engine instance
# itself — `import db.engine as db_engine` would bind to the
...
def db(db_session_factory, monkeypatch):
    ...
    # auth.py::_token_version_is_current does `from db.engine
    # import get_session` INSIDE the function body — a local import
    # re-resolves db.engine.get_session fresh on every call, so
    # it must be patched at the source module, not on api.auth
