...
from db.models import CustomStrategy, CustomStrategyPosition, FnoBhavcopy
...
class TestIvRankCondition:
    def test_dispatches_to_compute_iv_rank_and_compares(self):
        with patch("utils.iv_rank.compute_iv_rank", return_value=80.0):
    ...
    def test_none_rank_never_triggers(self):
        with patch("utils.iv_rank.compute_iv_rank", return_value=None):
...
class FakeOrderBroker:
    ...
    def place_buy_order(self, instrument_token, quantity, order_type, tag, user_id, product="NRML", is_close=False):
        self.orders.append(("BUY", instrument_token, quantity))
        return "ORD-1"

    def place_sell_order(self, instrument_token, quantity, order_type, tag, user_id, product="NRML", is_close=False):
...
class TestTryExitSplit:
    def test_individually_managed_leg_closes_independently_of_combined_leg(self, position_session, monkeypatch):
        ...
        monkeypatch.setattr(
            "utils.instrument_cache.InstrumentCache.get_or_refresh",
