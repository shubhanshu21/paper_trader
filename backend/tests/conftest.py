...
`backend` is installed editable (pip install -e . — see pyproject.toml),
so `from X import Y` resolves normally with no sys.path hack needed
here — backend/ is a flat package root (api/, db/, strategies/, etc.
live directly inside it, no automate/src wrapper).
...
import db.models  # noqa: F401 — import registers every model's table on Base.metadata before create_all() below
...
def db_session_factory(_test_engine):
    ...
    # expire_on_commit=False matches db.engine.SessionLocal's own
...
def _never_touch_real_notifications(db_session_factory, monkeypatch):
    ...
    db.engine import SessionLocal` (re-resolved fresh on every
    ...
    Patching db.engine.SessionLocal here redirects any such
    ...
    # shadows the `db.engine` package ATTRIBUTE with the Engine
    # instance itself — `import db.engine as db_engine` would
