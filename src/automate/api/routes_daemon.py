...
(`python3 -m automate.cli.run_daemon`) and the API as two independent,
...
# src/automate/api/routes_daemon.py -> up 4 levels to the repo root, same
# anchor backtest/__main__.py uses — needed so PID_FILE/logs resolve the
# same absolute path regardless of the API process's own cwd.
...
def _spawn_daemon() -> None:
    ...
    with open(LOG_FILE, "a") as log_fh:
        subprocess.Popen(
            [sys.executable, "-m", "automate.cli.run_daemon"],
            cwd=str(REPO_ROOT),
            stdout=log_fh,
            stderr=log_fh,
