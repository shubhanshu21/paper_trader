...
Cache strategy:
  ...
  - All cache files are auto-deleted when older than MAX_AGE_DAYS.
...
# Keep cache files for this many days before deleting
MAX_AGE_DAYS = 5
    ...
    def _cleanup_old_cache(self) -> None:
        ...
        for f in self.cache_dir.glob("upstox_instruments_*.csv"):
            if f.stat().st_mtime < cutoff:
                f.unlink()
                log.debug("Deleted stale cache file: %s", f.name)
