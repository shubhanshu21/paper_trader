...
automate.api.live_greeks's namespace — patching
automate.db.engine.SessionLocal (as test_wallet_real_margin.py does for
wallet.py, which instead does a LOCAL `from automate.db.engine import
get_session` import inside the function body, reading the name at call
time) would NOT reach it. Must patch automate.api.live_greeks.SessionLocal
...
class TestAggregatesAcrossStrategies:
    def test_sums_net_greeks_across_two_strategies_and_reports_per_strategy_breakdown(self, session_factory, monkeypatch):
        ...
        monkeypatch.setattr(
            "automate.api.custom_strategy_scheduler._get_brokers",
        ...
        monkeypatch.setattr(
            "automate.utils.instrument_cache.InstrumentCache.resolve_nearest_future_key",
    ...
    def test_broker_not_ready_returns_message_not_error(self, session_factory, monkeypatch):
        ...
        monkeypatch.setattr("automate.api.custom_strategy_scheduler._get_brokers", lambda: None)
