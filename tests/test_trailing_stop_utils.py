...
"""
from automate.utils.trailing_stop import advance_trailing_stop, exit_transaction_type, stop_triggered


class TestAdvanceTrailingStopBuySide:
    def test_first_tick_seeds_highest_and_stop(self):
        highest, lowest, stop, advanced = advance_trailing_stop("BUY", 100.0, 5.0, "points", None, None, None)
        assert highest == 100.0
        assert stop == 95.0
        assert advanced is True

    def test_new_high_ratchets_the_stop_up(self):
        highest, lowest, stop, advanced = advance_trailing_stop("BUY", 110.0, 5.0, "points", 100.0, None, 95.0)
        assert highest == 110.0
        assert stop == 105.0
        assert advanced is True

    def test_a_dip_does_not_loosen_the_stop(self):
        highest, lowest, stop, advanced = advance_trailing_stop("BUY", 102.0, 5.0, "points", 110.0, None, 105.0)
        assert highest == 110.0
        assert stop == 105.0
...
class TestAdvanceTrailingStopSellSide:
    def test_first_tick_seeds_lowest_and_stop(self):
        highest, lowest, stop, advanced = advance_trailing_stop("SELL", 100.0, 5.0, "points", None, None, None)
        assert lowest == 100.0
        assert stop == 105.0
        assert advanced is True

    def test_new_low_ratchets_the_stop_down(self):
        highest, lowest, stop, advanced = advance_trailing_stop("SELL", 90.0, 5.0, "points", None, 100.0, 105.0)
        assert lowest == 90.0
        assert stop == 95.0
        assert advanced is True

    def test_a_rally_does_not_loosen_the_stop(self):
        highest, lowest, stop, advanced = advance_trailing_stop("SELL", 98.0, 5.0, "points", None, 90.0, 95.0)
        assert lowest == 90.0
        assert stop == 95.0
        assert advanced is False
