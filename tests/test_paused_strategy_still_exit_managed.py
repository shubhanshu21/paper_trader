...
class TestPausedStrategyTick:
    ...
    def test_open_leg_still_gets_exit_checked_using_its_own_mode(self, db_session, monkeypatch):
        ...
        monkeypatch.setattr(
            "automate.api.custom_strategy_scheduler._is_leg_for_symbol", lambda key, sym: True,
        )
    ...
    def test_never_enters_a_new_position_while_paused(self, db_session, monkeypatch):
        ...
        called = {"entry": False}
        monkeypatch.setattr(
            "automate.api.custom_strategy_scheduler._try_entry",
...
class TestActiveStrategyTickUnchanged:
    def test_live_strategy_still_runs_entry_and_exit(self, db_session, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr("automate.api.custom_strategy_scheduler._try_exit", lambda *a, **k: calls.append("exit"))
        monkeypatch.setattr("automate.api.custom_strategy_scheduler._try_entry", lambda *a, **k: calls.append("entry"))
